...
def get_submenu_service():
    submenu_repository = SubmenuRepository(session=Depends(get_async_session))
    cache_repository = CacheRepository(redis_url="redis://localhost")
    return SubmenuService(submenu_repository=submenu_repository, cache_repository=cache_repository)
...
async def update_submenu(menu_id: UUID, submenu_id: UUID, submenu_data: CreateEditSubmenuModel, submenu_service: SubmenuService = Depends(get_submenu_service)):
    await submenu_service.update_submenu(menu_id, submenu_id, submenu_data.model_dump())
